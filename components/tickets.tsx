  ...
  return (
    <section id="tickets" className="py-12 md:py-20 lg:py-32 bg-gradient-to-br from-slate-900 via-blue-900/90 to-slate-900 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {isEarlyBirdActive && (
          <div className="max-w-4xl mx-auto mb-16 md:mb-20">
            <div className="relative group">
              ...
              <div className="relative bg-white/5 backdrop-blur-sm border border-orange-500/30 rounded-3xl p-6 md:p-8 shadow-2xl">
                <div className="flex items-center justify-center gap-3 mb-4 md:mb-6">
                  <Clock className="w-5 h-5 md:w-6 md:h-6 text-orange-400 animate-pulse" />
                  <span className="text-orange-400 font-black text-lg md:text-xl">Early Bird Special Ends In:</span>
                </div>
                <div className="grid grid-cols-4 gap-3 md:gap-6 max-w-lg mx-auto mb-4 md:mb-6">
                  {[
                    ...
                    <div key={index} className="text-center group">
                      <div className="relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-orange-500 to-yellow-500 rounded-2xl blur-lg opacity-50 group-hover:opacity-70 transition-opacity duration-300"></div>
                        <div className="relative bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-2xl p-2 md:p-4 mb-2 md:mb-3 shadow-xl transform group-hover:scale-105 transition-transform duration-300">
                          <span className="text-xl md:text-3xl font-black">{item.value.toString().padStart(2, "0")}</span>
                        </div>
                      </div>
                ...
                <div className="text-center">
                  <Badge className="bg-gradient-to-r from-green-500 to-emerald-500 text-white border-0 text-sm md:text-base font-bold px-4 md:px-6 py-2 shadow-lg">
        ...
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto mb-16 md:mb-20">
          {ticketTypes.map((ticket, index) => (
            ...
            >
              <Card
                className={`relative border-2 transition-all duration-700 hover:scale-105 h-full group flex flex-col ${
                  ticket.popular
              ...
              >
                ...
                {ticket.popular && (
                  <div className="absolute -top-4 md:-top-6 left-1/2 transform -translate-x-1/2 z-20">
                    <Badge className="bg-gradient-to-r from-orange-500 to-yellow-500 text-white border-0 px-4 md:px-6 py-2 text-sm md:text-base font-black shadow-xl animate-pulse">
                ...
                <CardHeader className="text-center pb-4 md:pb-6 relative flex-shrink-0">
                  {/* Floating sparkles */}
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-700 delay-200">
                    <Sparkles className="w-4 h-4 md:w-5 md:h-5 text-orange-400 animate-twinkle" />
                  </div>
                  
                  <div className="mb-4 md:mb-6">
                    <div className={`w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 md:mb-6 rounded-full ${ticket.badgeColor} flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500`}>
                      <ticket.icon className="w-8 h-8 md:w-10 md:h-10 text-white" />
                    </div>
                    <Badge className={`${ticket.badgeColor} text-white border-0 mb-4 md:mb-6 text-sm md:text-base font-bold px-3 md:px-4 py-1 md:py-2 shadow-lg`}>
                      {ticket.badge}
                    </Badge>
                  </div>
                  
                  <CardTitle className="text-2xl md:text-3xl font-black text-white mb-3 md:mb-4 group-hover:text-orange-400 transition-colors duration-500">
                    {ticket.name}
                  </CardTitle>
                  ...
                  <div className="flex items-center justify-center gap-2 md:gap-3 mb-3 md:mb-4">
                    {isEarlyBirdActive ? (
                      <>
                        <span className="text-3xl md:text-5xl font-black bg-gradient-to-r from-orange-400 to-yellow-500 bg-clip-text text-transparent">
                          {ticket.earlyBirdPrice}
                        </span>
                        <span className="text-lg md:text-xl text-gray-400 line-through">{ticket.price}</span>
                      </>
                    ) : (
                      <span className="text-3xl md:text-5xl font-black bg-gradient-to-r from-orange-400 to-yellow-500 bg-clip-text text-transparent">
                        {ticket.price}
                  ...
                  <p className="text-sm md:text-base text-gray-300 leading-relaxed group-hover:text-white transition-colors duration-500">
                    {ticket.description}
                  </p>
                </CardHeader>

                <CardContent className="space-y-6 md:space-y-8 flex-1 flex flex-col p-4 md:p-6">
                  <ul className="space-y-3 md:space-y-4 flex-1">
                    {ticket.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-start gap-3 group/item">
                        <CheckCircle className="w-5 h-5 md:w-6 md:h-6 text-green-400 mt-0.5 flex-shrink-0 group-hover/item:scale-110 transition-transform duration-300" />
                        <span className="text-sm md:text-base text-gray-300 leading-relaxed group-hover:text-white transition-colors duration-300">
                          {feature}
                        </span>
                      </li>
                    ))}
                  ...
                  <Button
                    ...
                    className={`w-full font-black text-base md:text-lg py-3 md:py-4 transition-all duration-500 transform hover:scale-105 shadow-xl hover:shadow-2xl ${
                      ticket.popular
                  ...
                  >
                    {ticket.popular && <Zap className="w-4 h-4 md:w-5 md:h-5 mr-2" />}
                    Register Now
        ...
        <div className="text-center mb-12 md:mb-16">
          <h3 className="text-2xl md:text-3xl lg:text-4xl font-black text-white mb-8 md:mb-12">
            Join us on{" "}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12 max-w-4xl mx-auto">
            {[
              ...
              <div key={index} className="relative group">
                ...
                <div className="relative bg-white/5 backdrop-blur-sm border border-white/20 rounded-2xl p-4 md:p-6 group-hover:border-orange-500/30 transition-all duration-500">
                  <item.icon className="w-6 h-6 md:w-8 md:h-8 text-orange-400 mx-auto mb-3 md:mb-4 group-hover:scale-110 transition-transform duration-300" />
                  <span className="font-bold text-white text-sm md:text-lg group-hover:text-orange-400 transition-colors duration-300">
                    {item.label}
                  </span>
                </div>
              </div>
          ...
          <div className="relative group max-w-2xl mx-auto">
            ...
            <div className="relative bg-white/5 backdrop-blur-sm border border-white/20 rounded-2xl p-4 md:p-6 group-hover:border-orange-500/30 transition-all duration-500">
              <div className="flex items-center justify-center gap-3">
                ...
                <span className="font-bold text-white text-sm md:text-xl">UP Cebu Performing Arts Hall, Gorordo Avenue, Lahug</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="text-center">
          <div className="relative inline-block mb-4">
            ...
            <Badge className="relative bg-gradient-to-r from-green-500 to-emerald-500 text-white border-0 text-base md:text-lg font-bold px-4 md:px-6 py-2 md:py-3 shadow-xl">
              {isEarlyBirdActive
